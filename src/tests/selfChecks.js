...
export function runSelfChecks({ clients, quotes, packages, services, supplies, promotions = [], quoteViews, packageMetrics, serviceMetrics, calendarEntries }) {
  ...
  const hasValidBaseData = (quote) => quote.quoteType === "Base" && activePackageIds.has(quote.packageId);
  ...
  const hasCompleteSnapshot = (quote) =>
    ...
    (quote.quoteType === "Base" ? quote.calculationSnapshot.packageSnapshot : quote.calculationSnapshot.packageSnapshot === null);
